...
def collect_historical_transfers(
    ...
    Priority:
    1. Primary: Solscan token transfer API (sort_order="asc", sort_by="block_time").
       Reads transactions starting directly from token genesis in ascending chronological order.
       Verifies ordering from response timestamps.
    2. Fallback: Native Solana RPC getSignaturesForAddress with bounded pagination.
       If max_pages is reached before genesis, flags candidate_discovery_complete=False
       and candidate_discovery_truncated=True.
    ...
    # =========================================================================
    # 1. PRIMARY SOURCE: Solscan Ascending (sort_order="asc")
    # =========================================================================
